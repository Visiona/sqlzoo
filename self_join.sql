...
SELECT y.name, a.company, a.num FROM stops x
JOIN route a ON a.stop = x.id
JOIN route b ON (a.company = b.company AND a.num = b.num)
...
SELECT  DISTINCT a.num, a.company, y.name, c.num, c.company FROM route a
JOIN route b ON (a.company = b.company AND a.num = b.num)
JOIN ( route c JOIN route d ON (c.company = d.company AND c.num = d.num))
JOIN stops x ON a.stop = x.id
JOIN stops y ON b.stop = y.id
JOIN stops yy ON c.stop = yy.id
JOIN stops z ON d.stop = z.id

WHERE x.name = 'Craiglockhart'
AND z.name = 'Sighthill'
AND y.name = yy.name
ORDER BY LENGTH(a.num), b.num, y.id, LENGTH(c.num), d.num
